...
- **Scanned at:** 2026-05-22T03:00:12.394Z
- **Probe version:** 1.0.0
...
Sent 12 failed login attempts; no 429 observed. Brute-force protection is missing or misconfigured.
...
### 🔵 Probe could not log in (subsequent auth checks skipped)

- **ID:** `auth-login-failed`
- **Severity:** info

Login to http://localhost:3000/api/auth/login with the configured credentials did not return a session_id cookie. Check that the dev account exists and CSRF flow is functional.
...
### 🔵 Input probe could not authenticate (skipped stored/reflected tests)

- **ID:** `input-login-failed`
- **Severity:** info

Login failed; input probe runs only the unauthenticated reflected tests below.

## websocket

### 🔵 WebSocket probe could not log in (authenticated tests skipped)

- **ID:** `ws-auth-login-failed`
- **Severity:** info

Login failed; only unauthenticated WS tests ran.
...
### 🟢 /collaboration WebSocket rejects malicious Origin header

- **ID:** `ws-collab-rejects-evil-origin`
- **Severity:** ok

Upgrade carrying Origin: https://evil.example.com was refused at handshake (closeCode=1006). CSWSH defense in place.
...
### 🟢 /events WebSocket rejects malicious Origin header

- **ID:** `ws-events-rejects-evil-origin`
- **Severity:** ok

Upgrade carrying Origin: https://evil.example.com was refused (closeCode=1006).
