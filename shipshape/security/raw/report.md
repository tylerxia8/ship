...
- **Scanned at:** 2026-05-21T23:03:04.404Z
- **Probe version:** 1.0.0
...
### 🔴 Server unhealthy after sending 11 MB binary payload on WS

- **ID:** `ws-malformed-server-crash-11-mb-binary-payload`
- **Severity:** critical
- **CWE:** CWE-20 (Improper Input Validation)

Sent 11 MB binary payload; /health failed/timed out immediately after. Likely server crash or hang triggered by the malformed frame.
...
```
{
  "frameRejected": true,
  "closeCode": 1009,
  "serverAliveAfter": false
}
```

### 🔴 Server unhealthy after sending text frame to binary Yjs endpoint on WS

- **ID:** `ws-malformed-server-crash-text-frame-to-binary-yjs-endpoint`
- **Severity:** critical
- **CWE:** CWE-20 (Improper Input Validation)

Sent text frame to binary Yjs endpoint; /health failed/timed out immediately after. Likely server crash or hang triggered by the malformed frame.
...
The same issue exists in `OrderedObjParser.js:81` (`addExternalEntities`), and in t

**Evidence:**
```
{
  "module": "fast-xml-parser",
  "vulnerable_versions": ">=5.0.0 <5.3.5",
  "patched_versions": ">=5.3.5",
  "url": "https://github.com/advisories/GHSA-m7jm-9gc2-mpf2"
}
```

**Remediation:** Upgrade to version 5.3.5 or later

### 🔴 [protobufjs] Arbitrary code execution in protobufjs
...
**Remediation:** Upgrade to version 7.5.5 or later
...
### 🟠 [fast-xml-parser] fast-xml-parser affected by DoS through entity expansion in DOCTYPE (no expansion limit)
...
The problem is in `replaceEntitiesValue()` inside `OrderedObjParser.js`. It repeatedly runs `val.replace()` in a loop, without any checks on total output size or execution cost. As the entity grows or th

**Evidence:**
```
{
  "module": "fast-xml-parser",
  "vulnerable_versions": ">=5.0.0 <5.3.6",
  "patched_versions": ">=5.3.6",
...
### 🟠 [fast-xml-parser] fast-xml-parser affected by numeric entity expansion bypassing all entity expansion limits (incomplete fix for CVE-2026-26278)
...
1. **Lines 638-670**: DOCTYPE entities — expansion counting with `entityExpansionCount` and `currentExpandedLength` tracking. This was the CVE-2026

**Evidence:**
```
{
  "module": "fast-xml-parser",
  "vulnerable_versions": ">=5.0.0 <5.5.6",
  "patched_versions": ">=5.5.6",
  "url": "https://github.com/advisories/GHSA-8gc5-j5rx-235r"
}
```

**Remediation:** Upgrade to version 5.5.6 or later
...
### 🟠 [protobufjs] protobuf.js: Code injection through bytes field defaults in generated toObject code
...
```
{
  "module": "protobufjs",
  "vulnerable_versions": "<=7.5.5",
  "patched_versions": ">=7.5.6",
  "url": "https://github.com/advisories/GHSA-66ff-xgx4-vchm"
}
```

**Remediation:** Upgrade to version 7.5.6 or later

### 🟠 [protobufjs] protobuf.js: Code generation gadget after prototype pollution
...
- The polluted property must affect protobufjs internal typ
...
### 🟠 [protobufjs] protobuf.js: Process-wide denial of service through unsafe option paths
...
- The application must allow an attacker to control or 
...
### 🟠 [protobufjs] protobuf.js: Denial of service through unbounded protobuf recursion
...
Avoid decoding untrusted protobuf binary data with affected versions. If immediate upgrade is not possible, reject excessively neste
...
### 🟡 [fast-xml-parser] Entity Expansion Limits Bypassed When Set to Zero Due to JavaScript Falsy Evaluation in fast-xml-parser
...
However, `DocTypeReader.js` uses truthy evaluation to check these limits. Because `0` is falsy in JavaScript, the entire guard expression short-circui

**Evidence:**
```
{
  "module": "fast-xml-parser",
  "vulnerable_versions": ">=5.0.0 <5.5.7",
  "patched_versions": ">=5.5.7",
  "url": "https://github.com/advisories/GHSA-jp2q-39xq-3w4g"
}
```

**Remediation:** Upgrade to version 5.5.7 or later
...
{
  "module": "fast-xml-parser",
  "vulnerable_versions": "<5.7.0",
  "patched_versions": ">=5.7.0",
  "url": "https://github.com/advisories/GHSA-gh4j-gqv2-49f6"
}
```

**Remediation:** Upgrade to version 5.7.0 or later
...
### 🟡 [protobufjs] protobuf.js: Denial of service from crafted field names in generated code
...
- The crafted input must define a field
...
### 🟡 [protobufjs] protobuf.js: Prototype injection in generated message constructors
...
### 🟡 [@protobufjs/utf8] protobufjs has overlong UTF-8 decoding
...
The practical impact depends on downstream application validation and how decoded strings are used. Node.js Buffer-backed decoding paths are not directly affected when they use Node's native UTF

**Evidence:**
```
{
  "module": "@protobufjs/utf8",
...
### 🟡 [protobufjs] protobufjs has overlong UTF-8 decoding
...
### 🟡 [protobufjs] protobufjs: Denial of Service via unbounded recursive JSON descriptor expansion
...
Avoid loading untrusted protobuf JSON descriptors with affected versions. If immediate upgrade is not possible,
...
```
{
  ...
  "patched_versions": ">=7.5.8",
  "url": "https://github.com/advisories/GHSA-jggg-4jg4-v7c6"
}
```

**Remediation:** Upgrade to version 7.5.8 or later
...
{
  "module": "fast-xml-parser",
  "vulnerable_versions": ">=5.0.0 <5.3.8",
  "patched_versions": ">=5.3.8",
  "url": "https://github.com/advisories/GHSA-fj3w-jwp8-x2g3"
}
```

**Remediation:** Upgrade to version 5.3.8 or later
...
The seeded dev account has is_super_admin=true. Recording responses for super-admin routes. Horizontal privilege-escalation testing (regular user → admin) would need a second seed account.
...
Probed non-JSON body, unknown route, and invalid UUID — none returned a Node.js stack trace, file path, or framework internals in the response body.
