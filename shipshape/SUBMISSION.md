...
8. **`scrollable-region-focusable` on `/dashboard` and `/team/directory`.** Cat 7 measurement (full WCAG 2.1 AA axe scan) flagged two `<div>` elements with `overflow:auto` that can't be scrolled with the keyboard — WCAG 2.1.1 (Keyboard) failure, serious impact. Fix is `tabindex="0"` + `role="region"` + `aria-label` on each, ~6 lines across two files. Outside the original Cat 7 color-contrast scope but a one-PR cleanup.
9. **`select-name` violation on `/settings` (10 nodes, critical).** The workspace-members table's role-select column renders 10 `<select>` elements without accessible names — WCAG 4.1.2 (Name, Role, Value). Source: [web/src/pages/WorkspaceSettings.tsx:324-336](../web/src/pages/WorkspaceSettings.tsx#L324-L336). Fix: add `aria-label={`Role for ${member.email}`}` to the select. ~2 lines. Missed by the original Cat 7 scan because the SR-curated rule filter saw the page in a less-hydrated state.
