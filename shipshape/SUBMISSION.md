...
|---|---|
| **Measurable improvement (40%)** | Every category hit its brief target with reproducible before/after numbers. Raw measurement output (autocannon JSON, EXPLAIN ANALYZE, axe scans, bundle treemaps) is committed under [shipshape/audit/raw/](audit/raw/) and [shipshape/improvements/raw/](improvements/raw/) so reviewers can rerun. Seven of eight categories (Cat 1, Cat 2, Cat 3, Cat 4, Cat 5, Cat 7, Cat 8) exceeded the target by ≥2×. |
...
- **Vercel + Render + Neon Terraform — `terraform apply` SUCCEEDED on 2026-05-22.** [terraform/render-vercel-neon/](../terraform/render-vercel-neon/) ran end-to-end against real provider APIs and stood up all five resources (`neon_project.ship damp-wind-20568016`, `render_web_service.api srv-d88bgo37uimc73bb1lvg`, `vercel_project.web prj_V1M4R1YClxXMG7o3WFapkLmRqYkQ`, `vercel_project_domain.web ship-shipshape.vercel.app`, `random_id.session_secret`). The Render-hosted API responded `HTTP 200` on `/health` against the Terraform-provisioned Neon Postgres database. Seven module fixes landed during the iterative apply — each driven by a real provider-API error and documented in [shipshape/security/raw-prod/terraform-apply-evidence.md](security/raw-prod/terraform-apply-evidence.md). Resources were torn down with `terraform destroy` immediately after artifact capture (total cost < $0.05; Render's `starter` tier billed by the second). One non-blocking limitation surfaced: `vercel_project` alone doesn't trigger a first deploy without the Vercel GitHub App installed on the repo — that's a Vercel-side webhook concern, not a TF defect, and the evidence file documents three workarounds.
- **Dockerfile + docker-compose.prod-mirror.yml — `up --build` SUCCEEDED + 5 Cat 8 smoke tests pass on 2026-05-22.** After recovering ~22 GB of disk by clearing the stale Docker WSL VHDX, the multi-stage Dockerfile built clean from empty cache (~2-3 min: Node 20-slim pull → pnpm install → tsc → final slim image), Postgres 16 + the api container came up healthy, and all 5 Cat 8 smoke tests pass against the running stack: `/health` 200; malformed JSON → `{success:false, error:{code:"VALIDATION_ERROR", message:...}}` with HTTP 400 (Fix #3 envelope, no stack leak); evil-origin CORS preflight gets `Access-Control-Allow-Origin: http://localhost:5173` (not the evil origin); WS handshake from evil origin → 403 Forbidden (Fix #4); all hardening headers (CSP, HSTS, X-Frame-Options, COOP, Referrer-Policy, rate-limit) present on responses. Full evidence at [shipshape/security/raw-prod/docker-prod-mirror-smoke-2026-05-22.md](security/raw-prod/docker-prod-mirror-smoke-2026-05-22.md). One known prod-mirror divergence noted: the migration step exits 1 because Postgres 16's default image doesn't enable SSL but Ship's pool config defaults to `?sslmode=require` (matches Render/Neon prod) — diagnostic prints exactly that, and the server starts anyway because the CMD uses `;` not `&&`.
...
| AI cost analysis | ✅ | [shipshape/ai-cost-analysis.md](ai-cost-analysis.md) — anchored spend estimate + actual 143,814-token Console tally + 3-section reflection |
| Deployed application | ✅ | Web: [ship-henna.vercel.app](https://ship-henna.vercel.app) · API: `ship-api-76ez.onrender.com` · DB: Neon. Production Cat 8 verification: [security/raw-prod/verification.md](security/raw-prod/verification.md) (12/12 ok, 2026-05-22). Codified as Terraform at [terraform/render-vercel-neon/](../terraform/render-vercel-neon/) (apply-verified, evidence: [terraform-apply-evidence.md](security/raw-prod/terraform-apply-evidence.md)) and as a production-mirror compose at [docker-compose.prod-mirror.yml](../docker-compose.prod-mirror.yml) (build-verified, evidence: [docker-prod-mirror-smoke-2026-05-22.md](security/raw-prod/docker-prod-mirror-smoke-2026-05-22.md)). |
| Social post (X / LinkedIn) | ✅ | 3 drafts each + posting checklist in [shipshape/social-posts.md](social-posts.md) |
