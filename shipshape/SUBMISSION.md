...
- **Vercel + Render + Neon Terraform was not `terraform apply`-tested in this session.** [terraform/render-vercel-neon/](../terraform/render-vercel-neon/) codifies the live deploy with the correct provider blocks, resource graph, and `depends_on` ordering; however no `terraform init` was run against it during this submission (no Terraform binary on the writing workstation). The README's "Honest hedge — provider attribute shapes" section enumerates the three leaf-attribute names most likely to need surgical adjustment on first apply.
