...
- **Vercel + Render + Neon Terraform was not `terraform apply`-tested in this session.** [terraform/render-vercel-neon/](../terraform/render-vercel-neon/) codifies the live deploy with the correct provider blocks, resource graph, and `depends_on` ordering; however no `terraform init` was run against it during this submission (no Terraform binary on the writing workstation). **Static verification passes**: balanced HCL2 syntax across all 4 `.tf` files, every cross-resource reference resolves (11/11), every `var.X` reference is declared (12/12). See the "Static verification" section in [terraform/render-vercel-neon/README.md](../terraform/render-vercel-neon/README.md) for reproducible checks. The three leaf-attribute names most likely to need surgical adjustment on first apply are enumerated there.
