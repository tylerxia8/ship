...
Raw output: [raw/web-build-baseline.txt](raw/web-build-baseline.txt) _(in progress)_.

### Findings (preliminary, from package inspection)

- **Editor weight:** TipTap is fragmented into ~15 packages in [web/package.json](../../web/package.json) — `@tiptap/core`, `react`, `pm`, `starter-kit`, plus extensions (`code-block-lowlight`, `collaboration`, `collaboration-cursor`, `dropcursor`, `link`, `mention`, `placeholder`, `table*`, `task-*`, `suggestion`). Each is independently versioned (~2.10 / ~2.27 mismatch — investigate whether this fragments the dedupe).
- **Lowlight + diff-match-patch + emoji-picker-react** are big libraries by default. Each is a code-split candidate (lowlight in `code-block-lowlight` only, diff in `DiffViewer.tsx`, emoji-picker in `EmojiPicker.tsx`).
- **`@aws-sdk/*` is only in api/**, not web — good.
- **No bundle analyzer configured** today; nothing in `vite.config.ts` produces bundle stats.
- **No explicit `manualChunks`** in `vite.config.ts` — relying entirely on Vite/Rollup defaults.

### Improvement target

> -15% total or implement code splitting that reduces initial load by -20%.

Likely path: lazy-load `EmojiPicker`, `DiffViewer`, and probably the entire admin route. Plus `@tiptap/extension-code-block-lowlight` lazy-import (lowlight is big, not used on every doc).
