...
This is a one-page index so a reviewer can confirm each requirement is satisfied without scrolling the full 450-line [AUDIT_REPORT.md](AUDIT_REPORT.md). Line refs point into that file; raw-evidence refs point into [raw/](raw/).
...
|---|---|---|
| `shipshape/audit` | Diagnosis only — this report + raw measurement output | **Only** the `SHIPSHAPE_AUDIT=1` env flag in [api/src/app.ts](../../api/src/app.ts) — pure measurement instrumentation. No handler logic changed. Called out explicitly in [§ Cat 3 Methodology](AUDIT_REPORT.md#L189-L191). |
| `shipshape/01-type-safety` … `shipshape/07-accessibility` | One per category — fixes branch off `shipshape/audit` | Yes, deliberately separate so reviewers can `git diff shipshape/audit shipshape/0N-...` per category |
