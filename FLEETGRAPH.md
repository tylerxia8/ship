...
Each use case has an explicit Ship state, expected agent behavior, and trace/evidence pointer below. Two public LangSmith traces are shared because they are the clearest proof of different graph paths: a read-only on-demand path and a HITL/proactive-style path. Several use cases reuse those traces because they exercise the same compiled graph with different intent/fetch subsets; the exact fetched state is documented in the Ship State column.

| Use Case | Ship State That Triggers It | Agent Should Detect / Produce | Trace / Evidence |
|---|---|---|---|
| UC1 — Issue blocker traversal | Issue `fc466b06...` ("Create mobile app") opened in Ship; state=`backlog`, priority=`low`, estimate=`40h`, user asks "is this blocking anything?" | Fetch scoped issue + associations, classify `blocker_check`, return cited blocker summary without human gate. | [trace 1](https://smith.langchain.com/public/aed63ea9-170a-4042-820c-c4e811800ebc/r) |
| UC2 — Sprint slip / planning risk | Sprint `09e44014...` ("Week 16") opened in Ship; `confidence_score=42.4%`, missing goals/criteria, user asks what is slipping. | Fetch sprint + adjacent work, produce risk finding, propose notify/comment actions, route through `human_gate`. | [trace 2](https://smith.langchain.com/public/cac0e57f-7436-4dd3-b360-3f0f2119fb93/r) |
| UC3 — Overloaded people | Resource/person-oriented query against seeded workspace containing 11 people and 104 issues with estimate/capacity properties. | Activate `load_check`, fetch people/issues/sprint associations, rank people by assigned estimate vs capacity, gate any reassignment proposal. | Same graph implementation; `fetch_load` path documented in Graph Diagram. Public path proof: [trace 2](https://smith.langchain.com/public/cac0e57f-7436-4dd3-b360-3f0f2119fb93/r) for gated action behavior. |
| UC4 — Proactive blocker/stale chain | Active sprint scanned with no user message; sprint has stale or low-confidence issue/project signals worth surfacing. | Fast-path `proactive_scan`, decide whether the finding is worth surfacing, persist a scoped finding/notification card. | Test case 3 below; production evidence screenshot [fleetgraph-production-week17.png](shipshape/fleetgraph-evidence/fleetgraph-production-week17.png). Public path proof: [trace 2](https://smith.langchain.com/public/cac0e57f-7436-4dd3-b360-3f0f2119fb93/r). |
| UC5 — Change since retro | Program/sprint/page query with history-like properties and recent adjacent document activity. | Activate `diff_query`, run `fetch_history` + `fetch_activity`, summarize meaningful changes, gate any retro update. | Same graph implementation; `fetch_history` path documented in Graph Diagram. Public path proof: [trace 1](https://smith.langchain.com/public/aed63ea9-170a-4042-820c-c4e811800ebc/r) for read-only answer behavior. |
| UC6 — Sprint overcommit warning | Newly planned/active sprint scanned on next poll/manual scan; planned issue estimate total is compared to team capacity. | Activate proactive reasoning over sprint load, surface capacity/overcommit finding, propose de-scope/rebalance as gated action. | Production manual scan test case 7 below; same persisted finding path as proactive mode. Public path proof: [trace 2](https://smith.langchain.com/public/cac0e57f-7436-4dd3-b360-3f0f2119fb93/r). |
