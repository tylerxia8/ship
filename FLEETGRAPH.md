...
Final trace hygiene: each row below should have its own public LangSmith URL before final submission. Run `pnpm --filter @ship/agent trace-matrix` after the final evidence runs to find the matching run IDs, then share each run from LangSmith and paste the distinct public URLs here. Rows marked `Pending unique public URL` are intentionally not borrowing the representative links.
