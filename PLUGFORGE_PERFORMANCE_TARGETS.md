...
| TTFE drill runtime in CI P95 < 60s | CI runs `corepack.cmd pnpm drill ttfe`; any run above the default `TTFE_TARGET_MS=60000` fails. P95 is computed from CI history, and every individual proof emits `elapsed_ms` for aggregation. |
| TTFE on a clean machine docs-only <= 30 minutes real elapsed | The same harness supports `TTFE_TARGET_MS=1800000` for the human/docs-only challenge path from a clean checkout. |
| Webhook signature verification, SDK helper < 1ms per call | `corepack.cmd pnpm plugforge:perf` runs `scripts/plugforge-signature-perf.mjs`, verifying 50,000 signed payloads after warmup and failing if average `verifyWebhook` time is >= `SHIP_SIGNATURE_VERIFY_TARGET_MS`, default `1`. |
| Drill flake rate over 20 consecutive CI runs = 0% | `corepack.cmd pnpm plugforge:flake` runs `corepack.cmd pnpm plugforge:flows` twenty times by default (`PLUGFORGE_FLAKE_RUNS=20`) and fails on any failure. Latest local proof on 2026-06-03 passed 20/20 runs with `failures: 0` and `flake_rate: 0`; CI uploads `test-results/plugforge-flake-proof.json`. Any flake is treated as a drill/platform bug. |
| SDK install size, production deps only < 250 KB minified + gzipped | `corepack.cmd pnpm plugforge:perf` runs `scripts/plugforge-sdk-size.mjs`, measuring built SDK JavaScript after conservative minification plus gzip and failing above `SHIP_SDK_GZIP_TARGET_BYTES`, default `256000`. The SDK has no production dependencies. |
