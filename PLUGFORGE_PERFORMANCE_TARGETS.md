...
| Drill flake rate over 20 consecutive CI runs = 0% | `corepack.cmd pnpm plugforge:flake` runs `corepack.cmd pnpm plugforge:flows` twenty times by default (`PLUGFORGE_FLAKE_RUNS=20`) and fails on any failure. Any flake is treated as a drill/platform bug. |
