...
| `SHIP_AGENT_CLIENT_ID` | preferred | OAuth app client ID for FleetGraph public API reads through Client Credentials |
| `SHIP_AGENT_CLIENT_SECRET` | preferred | OAuth app client secret for FleetGraph public API reads through Client Credentials |
| `SHIP_AGENT_CLIENT_SCOPE` | no (default `documents:read`) | Scope requested by the agent client credentials exchange |
| `SHIP_PUBLIC_API_TOKEN` | fallback | Pre-minted OAuth bearer token for FleetGraph document reads through `@ship/sdk` and `/api/v1` |
...
When `SHIP_AGENT_CLIENT_ID` and `SHIP_AGENT_CLIENT_SECRET` are set, FleetGraph
uses OAuth Client Credentials to mint a short-lived scoped token, then reads
documents through `@ship/sdk` against `/api/v1`. Those reads produce the same
OAuth app audit rows, scope checks, and rate-limit behavior as an external
integration. `SHIP_PUBLIC_API_TOKEN` remains a compatibility fallback for
captured demos. Association reads and finding writes still use the
service-account path until those surfaces are promoted into the public API.
